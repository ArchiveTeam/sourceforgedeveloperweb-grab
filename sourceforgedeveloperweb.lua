...
local item_patterns = {
  ["^https?://([^%.]+%.users%.sourceforge%.net/?.*)$"]="path",
  ["^https?://([^%.]+%.users%.sourceforge%.io/?.*)$"]="path",
  ["^https?://([^%.]+%.users%.sf%.net/?.*)$"]="path",
  ["^https?://([^%.]+%.users%.sf%.io/?.*)$"]="path"
}
...
set_item = function(url)
  found = find_item(url)
  if found then
    local newcontext = {["extract_urls"]={}}
    new_item_type = found["type"]
    new_item_value = found["value"]
    if new_item_type == "path" then
      local user = string.match(new_item_value, "^([^%.]+)")
      local path = string.match(new_item_value, "^[^/]+(.-)$")
      newcontext["user"] = user
      newcontext["path"] = path
      new_item_value = user .. ":" .. path
    end
...
allowed = function(url, parenturl)
  ...
  if not string.match(url, "^https?://[^/]*sourceforge%.net/")
    and not string.match(url, "^https?://[^/]*sf%.net/") then
...
wget.callbacks.download_child_p = function(urlpos, parent, depth, start_url_parsed, iri, verdict, reason)
  ...
  context["extract_urls"][parent["url"]] = true
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if string.match(url, "^https?://[^/]+/.") then
    local newurl = url
    while string.match(newurl, "^https?://[^/]+/.") do
      newurl = string.match(newurl, "^(https?://.+)/")
      check(newurl .. "/")
    end
  end

  local user, path = string.match(url, "^https?://([^%.]+)[^/]+(.-)$")
  if user and path then
    check(urlparse.absolute("//" .. user .. ".users.sourceforge.net" .. path))
    check(urlparse.absolute("//" .. user .. ".users.sf.net" .. path))
  end

  if allowed(url)
    and status_code < 300
    and (
      string.match(url, "%.s?html?$")
      or string.match(url, "/$")
      or context["extract_urls"][url]
    ) then
    html = read_file(file)
    for _, pattern in pairs({
      ...
      for user in string.gmatch(html, pattern) do
        check("https://" .. user .. ".users.sf.net/")
      end
    end
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["sourceforgedeveloperweb-38slub3phth5rlc0"] = discovered_items,
    ["urls-682pwq6dl24c88oq"] = discovered_outlinks
  }) do
